...
export function renderCard(m) {
  ...
  return `
    <div class="mailbox-card" data-address="${addr}" data-id="${m.id}">
      <label class="mailbox-select" title="选择邮箱">
      ...
      <div class="actions actions-static">
        <button class="btn-icon" data-action="copy" title="复制">📋</button>
        <button class="btn-icon" data-action="jump" title="查看邮件">📧</button>
        ...
        <button class="btn-icon" data-action="login" title="${m.can_login ? '禁止登录' : '允许登录'}">${m.can_login ? '🔐' : '🔓'}</button>
...
export function renderListItem(m) {
  ...
  return `
    <div class="mailbox-list-item" data-address="${addr}" data-id="${m.id}">
      <div class="pin-indicator" style="display:flex;align-items:center;gap:8px;">
        <input type="checkbox" class="mailbox-checkbox" data-address="${addr}" style="cursor:pointer;" title="选择邮箱" />
