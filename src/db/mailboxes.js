...
export async function countMailboxesByDomain(db, domain) {
  const normalized = String(domain || '').trim().toLowerCase();
  if (!normalized) return 0;
  try {
    const result = await db.prepare('SELECT COUNT(1) AS count FROM mailboxes WHERE domain = ?').bind(normalized).all();
    return Number(result?.results?.[0]?.count || 0);
  } catch (error) {
    console.error('统计域名邮箱数量失败:', error);
    return 0;
  }
}
