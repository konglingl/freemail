...
async function performFirstTimeSetup(db) {
  ...
  try {
    ...
    await ensureMailDomainsTable(db);
    await migrateMailDomainsFields(db);
    await createMailDomainIndexes(db);
    return;
  } catch (e) {
    // 有表不存在，继续初始化
 ...
 * @returns {Promise<void>}
 */
async function createMailDomainIndexes(db) {
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_mail_domains_status ON mail_domains(status);`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_mail_domains_kind_status ON mail_domains(kind, status);`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_mail_domains_dns_status ON mail_domains(dns_status);`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_mail_domains_restore_until ON mail_domains(restore_until);`);
}
...
async function migrateMailDomainsFields(db) {
  try {
    const columns = await db.prepare("PRAGMA table_info(mail_domains)").all();
    const columnNames = (columns.results || []).map(c => c.name);

    if (!columnNames.includes('dns_status')) {
      await db.exec("ALTER TABLE mail_domains ADD COLUMN dns_status TEXT DEFAULT 'active';");
    }
    if (!columnNames.includes('restore_until')) {
      await db.exec("ALTER TABLE mail_domains ADD COLUMN restore_until TEXT;");
    }
    if (!columnNames.includes('preserve')) {
      await db.exec("ALTER TABLE mail_domains ADD COLUMN preserve INTEGER DEFAULT 0;");
    }
    if (!columnNames.includes('retired_at')) {
      await db.exec("ALTER TABLE mail_domains ADD COLUMN retired_at TEXT;");
    }
    if (!columnNames.includes('last_error')) {
      await db.exec("ALTER TABLE mail_domains ADD COLUMN last_error TEXT;");
    }
  } catch (error) {
    console.error('mail_domains 字段迁移失败:', error);
  }
}
