...
import { ensureMailDomainSeeded } from './domain-rotation/service.js';
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // 解析邮件域名（前端展示使用 runtime 过滤结果，避免根域名继续暴露）
    let MAIL_DOMAINS = [];

    try {
      const url = new URL(request.url);
      if (url.pathname.startsWith('/api/') || url.pathname === '/' || url.pathname === '/index.html') {
        await ensureMailDomainSeeded(DB, env);
      }
      MAIL_DOMAINS = await getRuntimeMailDomains(DB, env);
    } catch (error) {
      console.error('mail_domains init failed:', error?.message || error);
      MAIL_DOMAINS = [];
    }
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      const { ensureMailDomainSeeded, runAutoRotationTick } = await import('./domain-rotation/service.js');
      await ensureMailDomainSeeded(DB, env);
      const result = await runAutoRotationTick(DB, env);
      console.log('scheduled auto rotation result:', JSON.stringify(result));
    } catch (error) {
      console.error('scheduled auto rotation failed:', error?.message || error);
    }
  },
