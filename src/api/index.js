...
import { handleMailDomainAdminApi } from './mailDomainAdmin.js';
...
export async function handleApiRequest(request, db, env, options = {
  ...
  // 域名管理 API
  response = await handleMailDomainAdminApi(request, db, env, url, path, options);
  if (response) return response;

  // 邮箱管理 API
  response = await handleMailboxesApi(request, db, env, url, path, options);
  if (response) return response;
