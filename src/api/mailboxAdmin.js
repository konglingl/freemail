    ...
    try {
      ...
      for (const [normalizedAddress, originalAddress] of addressMap.entries()) {
        try {
          ...
          const deleted = (deleteResult?.meta?.changes || 0) > 0;
          if (deleted) {
            invalidateMailboxCache(normalizedAddress);
            successCount++;
            results.push({ address: normalizedAddress, success: true, deleted: true });
          } else {
            failCount++;
            results.push({ address: normalizedAddress, success: false, error: '删除失败' });
          }
        } catch (e) {
          try { await db.exec('ROLLBACK'); } catch (_) { }
          failCount++;
          results.push({ address: originalAddress, success: false, error: String(e?.message || '删除失败') });
        }
      }
